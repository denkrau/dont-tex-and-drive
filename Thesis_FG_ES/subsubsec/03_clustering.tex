...
\begin{align}
Maske(x,y)=\left\{\begin{matrix}
1 \ \ \ \ falls \ BBox(x,y) \leq \frac{1}{N_{x}+N_{y}} \sum_{x=0}^{N_{x}}\sum_{y=0}^{N_{y}}BBox(x,y)  \\ 
0 \ \ \ \ falls \ BBox(x,y) > \frac{1}{N_{x}+N_{y}} \sum_{x=0}^{N_{x}}\sum_{y=0}^{N_{y}}BBox(x,y)
\end{matrix}\right.
\end{align}
